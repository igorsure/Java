...
public class Solution {
	...
	private boolean checkmate2(int x1, int y1, int x2, int y2) {
		...
		for(int ii=0; ii<64;ii++) {
			...
			for(int ii1=0; ii1<64;ii1++) {
				...
				//dumpBoard(brd);
				boolean mate=true;
				if(!ismate(x1,y1,brd,x2,y2)) mate=false;
				if(!ismate(x2,y2,brd,x1,y1)) mate=false;
				//printRes(new int[][]{{i1,j1},{i2,j2}});
				if (mate) {
					printRes(new int[][]{{i1,j1},{i2,j2}},brd);
					//dumpBoard(brd);
					return true;
				}
				brd[i2][j2]=0;
	...
	private void printRes(int[][] is,int[][] brd) {
	...
	private boolean ismate(int x1, int y1, int[][] brd,int x2,int y2) {
		...
		for(int dx=-1;dx<=1;dx++) {
			for(int dy=-1;dy<=1;dy++) {
				...
				boolean checked1=!ischeck(x2,y2,brd);
				brd[mx][my]=prevm;
				brd[x1][y1]=-1;
				if (checked) {
					return false;
		...
		for(int ii=0; ii<64;ii++) {
			...
			for(int ii1=0; ii1<64;ii1++) {
				...
				for(int ii2=0; ii2<64;ii2++) {
					...
					if(!ismate(x2,y2,brd,x1,y1)) mate=false;
					if (mate) {
						printRes(new int[][]{{i1,j1},{i2,j2},{i3,j3}},brd);
						//dumpBoard(brd);
						return;
					}
					brd[i3][j3]=0;
